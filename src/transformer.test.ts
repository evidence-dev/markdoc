  ...
  describe('attributes', function () {
    describe('string interpolation', function () {
      ...
      it('should not interpolate variables without $ prefix', function () {
        const node = createTestNode({ title: 'Hello {{name}} and {{$name}}' });
        const config = createTestConfig({ name: 'World' });
        
        const result = transformer.attributes(node, config);
        expect(result.title).toBe('Hello {{name}} and World');
      });

      it('should not interpolate nested variables without $ prefix', function () {
        const node = createTestNode({ title: 'Hello {{user.name}} and {{$user.name}}' });
        const config = createTestConfig({ user: { name: 'John' } });
        
        const result = transformer.attributes(node, config);
        expect(result.title).toBe('Hello {{user.name}} and John');
      });

      it('should not interpolate mixed syntax', function () {
        const node = createTestNode({ title: 'Hello {{name}} {{$name}} {{$user.name}} {{user.name}}' });
        const config = createTestConfig({ 
          name: 'World',
          user: { name: 'John' }
        });
        ...
        const result = transformer.attributes(node, config);
        expect(result.title).toBe('Hello {{name}} World John {{user.name}}');
      });
    });
  });
}); 